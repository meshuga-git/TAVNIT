...
2. Parse the raw data with the following command: python clustering.py <distance metric number>. The command asks for paths to folders with raw data, generates a new folder with parsed data, and creates the empty folder clustering_results for future results. This folder is created within the same superfolder that contains the raw data.
